...
def process_single_frame(nd2_file_path, position_idx, t_idx, z_idx, channel, vmin, vmax, save_path):
    """Process a single frame: read, adjust, and save as an 8-bit PNG."""
    try:
...
def process_nd2_images_multithreaded(nd2_file_path, output_dir, black_white_points_path, date, initials, progress_callback=None):
    """Process images from an ND2 file using multithreading."""
    ...
    with ND2File(nd2_file_path) as nd2_data:
            ...
            for t_idx in range(num_timepoints):
                for z_idx in range(num_stacks):
                    for channel in range(num_channels):
                        ...
                        tasks.append((position_idx, t_idx, z_idx, channel, vmin, vmax, save_path))
        ...
        with ProcessPoolExecutor(max_workers=os.cpu_count()) as executor:
            ...
            # Iterate in the order of completion
            for i, future in enumerate(as_completed(futures), 1):
                if progress_callback:
                    # e.g. "Converting image 1 of 6144"
